...
// Streams best bid/ask via Binance's combined @bookTicker WebSocket streams.
class BinanceMarketDataConnector : public IMarketDataConnector {
public:
    BinanceMarketDataConnector();
    ~BinanceMarketDataConnector() override;
...
private:
    ...
    std::unordered_map<std::string, std::string> nativeToCanonical_;  // lowercased native symbol -> canonical code
