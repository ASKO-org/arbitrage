...
OkxFutMarketDataConnector::OkxFutMarketDataConnector()
    : webSocket_(std::make_unique<ix::WebSocket>()),
      heartbeat_(std::chrono::seconds(20), [this] { webSocket_->send("ping"); }) {}

OkxFutMarketDataConnector::~OkxFutMarketDataConnector() { stop(); }

std::string OkxFutMarketDataConnector::exchangeName() const { return "OKX_Fut"; }

void OkxFutMarketDataConnector::subscribe(const std::vector<TrackedSymbol>& symbols) {
    symbols_ = symbols;
...
void OkxFutMarketDataConnector::setOnQuote(std::function<void(const Quote&)> onQuote) {
    onQuote_ = std::move(onQuote);
}

void OkxFutMarketDataConnector::start() {
...
void OkxFutMarketDataConnector::stop() {
    heartbeat_.stop();
    webSocket_->stop();
}

void OkxFutMarketDataConnector::sendSubscribe() {
...
void OkxFutMarketDataConnector::handleMessage(const std::string& payload) {
    if (!onQuote_) return;
