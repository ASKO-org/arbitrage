...
BitgetSpotMarketDataConnector::BitgetSpotMarketDataConnector()
    : webSocket_(std::make_unique<ix::WebSocket>()),
      heartbeat_(std::chrono::seconds(25), [this] { webSocket_->send("ping"); }) {}

BitgetSpotMarketDataConnector::~BitgetSpotMarketDataConnector() { stop(); }

std::string BitgetSpotMarketDataConnector::exchangeName() const { return "BITGET_Spot"; }

void BitgetSpotMarketDataConnector::subscribe(const std::vector<TrackedSymbol>& symbols) {
    symbols_ = symbols;
...
void BitgetSpotMarketDataConnector::setOnQuote(std::function<void(const Quote&)> onQuote) {
    onQuote_ = std::move(onQuote);
}

void BitgetSpotMarketDataConnector::start() {
...
void BitgetSpotMarketDataConnector::stop() {
    heartbeat_.stop();
    webSocket_->stop();
}

void BitgetSpotMarketDataConnector::sendSubscribe() {
    ...
    for (size_t offset = 0; offset < natives.size(); offset += kMaxSymbolsPerMessage) {
        ...
        for (size_t i = offset; i < std::min(offset + kMaxSymbolsPerMessage, natives.size()); ++i) {
            args.push_back(
                {{"instType", "SPOT"}, {"channel", "books1"}, {"instId", natives[i]}});
        }
        webSocket_->send(nlohmann::json{{"op", "subscribe"}, {"args", args}}.dump());
    }
}

void BitgetSpotMarketDataConnector::handleMessage(const std::string& payload) {
    if (!onQuote_) return;
    ...
    try {
        ...
        // Malformed or unexpected payload shape (e.g. the bare "pong" frame
        // already handled above) — drop it.
    }
}
